package com.example.wendy.utils;
...
public class DragImageView extends ImageView {
    ...
    private enum MODE {//NONE：无 DRAG：拖拽. ZOOM:缩放
        NONE, DRAG, ZOOM
    }
    ...
    public void setmActivity(Activity mActivity, boolean downClose) {
        this.mActivity = mActivity;
        this.downClose = downClose;
    }
    ...
    void onTouchMove(MotionEvent event) {
        ...
        if (mode == MODE.DRAG) {
            ...
            int disx = 0, change = 0;
            Log.i("test", "scale" + String.valueOf(scale_temp));
            if (scale_temp != 0) {
                disx = (int) (this.getWidth() * Math.abs(1 - scale_temp));
                change = left - (this.getLeft() - disx);
            } else {
                change = left;
                Log.i("test", "test");
            }
            Log.i("test", String.valueOf(change));
            int mark = 0;
